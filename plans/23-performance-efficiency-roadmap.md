...
**Implementation status:** A1 (stable PHI-safe spans and one request summary for chat, search, agent,
semantic retrieval, and structured execution) is complete. A2 is implemented under `eval/`: authenticated
router probes plus synthetic-seed retrieval fixtures report route accuracy, hit-rate@6, and MRR. Run
`node eval\run.mjs --suite full --smoke`; live gate validation requires the local stack and ingested seed.
