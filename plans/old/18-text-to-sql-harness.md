...
> Status: IMPLEMENTED (core path). Aggregation / distribution / listing questions are automatically
> routed to a dialect-aware NL→SQL pipeline and answered with **exact rows from one connected
> operational database** (Postgres, MySQL, or SQL Server). SQL is generated by a local Foundry model,
> emitted as plain SQL to avoid unsupported ONNX tool grammars, AST-validated as read-only, row/time
> limited, repaired at most once within one 30-second planning deadline, and persisted with its result
> provenance. Common counts, bounded listings, grouped counts, and date trends use a validated
> deterministic fast path; other questions use the local SQL model. The cached model is preloaded after
> execution-provider registration, and result summaries are deterministic (no second model call).
> Schema cards are created/refreshed with source lifecycle events
> and lazily backfilled for existing connected sources. Failure falls back to the ingested DocumentDB
> structured executor and then semantic retrieval. Retrieval controls remain server-owned and hidden
> from ordinary chat users. Curated/learned few-shot retrieval and hybrid vector + text schema ranking
> remain future accuracy enhancements.
...
few-shot lookup ──► `sql_examples` (validated question→SQL pairs, vector match)
   │
   ▼
generate (phi-4-mini, direct SQL, per-dialect template) ──► SQL text
   │
   ▼
...
execute (connector run_select, read-only, max rows, timeout) ── error ──► 1 repair pass
   │
   ▼
deterministic summary + SSE (sql/columns/rows/token) + audit log
   │
...
- System prompt = dialect rules + schema cards + 2–3 nearest few-shots (from `sql_examples` by
  ...
  `LIMIT/TOP` yourself.
- Output as one plain SQL statement (a single `sql` fence is also accepted). Constrained tool grammar is
  deliberately bypassed because the local ONNX variant rejects the `emit_sql` schema before generation.
- `ModelSpec`: `AgentKind::TextToSql` → alias `ONPREM_MODEL_TEXT2SQL=phi-4-mini-instruct`,
  `thinking:false`, `temperature:0`, `tools:false`, 256 output tokens, device `[Npu, Cpu]`. The cached
  model is loaded after execution-provider registration so graph setup happens in the background.
...
- `POST /agents/sql` (AuthUser): body `{ question, source_id? }`. SSE events:
  ...
  (deterministic result summary) → `done` | `error`. `/chat` reaches the same executor through the router
...
ONPREM_NL2SQL_PLAN_TIMEOUT_SECS=30        # one deadline shared by planning + one repair
