...
use crate::answer::{
    build_agg_planner_system, build_narration_user, NARRATION_SYSTEM_PROMPT,
};
...
pub async fn agent(
    ...
    // Resolve the effective kind. `auto` is intercepted here so `parse_kind`
    // never sees it; `classify_auto` never HTTP-errors (falls open to Chat).
    let resolved_kind: AgentKind = if kind.eq_ignore_ascii_case("auto") {
        classify_auto(state.inner(), &body.question).await
    } else {
        parse_kind(kind)?
    };
    ...
    let data: AgentData = match resolved_kind {
        ...
        AgentKind::HealthQuery | AgentKind::Trends => {
            ...
            let (rows, pipeline_docs) = execute::run(&state.db, &user, &validated).await?;
            ...
            AgentData::Structured { spec_json, rows_json, pipeline_json, narration_stream }
        }
...
/// Resolve the effective `AgentKind` for an `auto` request.
///
/// Delegates to the shared intent router (Tier 0 gate -> Tier 1 lexical -> Tier 2
/// model classify, fail-open to semantic) and maps the resulting route class to an
/// `AgentKind`. Never errors — a conversational/semantic/hybrid route resolves to
/// `AgentKind::Chat`, the safe grounded default. Route decisions are cached in the
/// shared `router_cache`, so an `auto` agent request and a `/chat` request for the
/// same question share Tier-2 work.
async fn classify_auto(state: &AppState, question: &str) -> AgentKind {
    let decision = crate::router::route(
        question,
        false, // /agents/auto is stateless w.r.t. router history hints
        &state.config,
        state.foundry().ok(),
        &state.router_cache,
    )
