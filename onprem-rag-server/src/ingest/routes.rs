...
//!
//! The job runs as a detached background task; the SSE route is a thin **poller**
//! (every 500 ms) over the `jobs` document. Polling keeps progress stateless and
//! multi-client friendly — any client can attach, and a dropped connection never
//! aborts the ingest.
...
pub async fn start_ingest(
    ...
    let spec = crate::connectors::routes::load_spec(&state.db, &state.config, &req.source_id).await?;
    ...
    audit::write_audit(
        &state.db, &user.id, &user.username, "ingest_started", &req.source_id,
...
/// may watch. Polls every 500 ms and stops on a terminal status.
///
...
pub async fn ingest_stream(state: &State<AppState>, _user: AuthUser, job: String) -> EventStream![] {
    let db = state.db.clone();
        ...
        loop {
            match db.jobs().find_one(doc! { "_id": &job }).await {
                Ok(Some(doc)) => {
                    ...
                    match status.as_str() {
                        "completed" | "partial" => {
                            // Rebuild the catalog now that new data is indexed.
...
pub async fn delete_ingest_connection(
    ...
    Ok(Json(serde_json::json!({ "tables_removed": removed as i64 })))
}
