// Renders the full agent conversation: persisted messages from the server query,
// then the optimistic pending pair (user + assistant) when a run is in flight.
// Mirror of chat/MessageList.tsx — uses AgentPending instead of PendingRun.
...
export default function MessageList({ persisted, pending, activeConvId }: MessageListProps) {
  ...
  // or phase transitions (e.g. rows arrive while answer is still empty).
  useEffect(() => {
    if (listRef.current) {
